...
_GENERATED_LEVER_MODULE = "perovskite_sim.autoloop.generated.lever"
...
from perovskite_sim.autoloop.promote import propose_promotion, apply_edit, revert_edit, is_promotable
...
def commit_generated_lever(target_path, lever, gap, hypothesis, verdicts, *, git_cwd=None):
    ...
    try:
        ...
        _git("add", rel)
        # ASCII pass/fail (not ✓/✗) so the commit message round-trips through any
        # terminal / git porcelain / log tooling without mojibake.
        gate_summary = " ".join(f"{v.name}:{'pass' if v.passed else 'fail'}" for v in verdicts)
...
def codegen_top_not_promotable(*, ledger_root: Path, outputs_root: Path, config_path,
    ...
    # Use is_promotable (filesystem-free) so routing never depends on config_path
    # existing — propose_promotion now raises on an absent config (Task 7c).
    candidates = [g for g in candidates if not is_promotable(_hyp(g), led)]
    if not candidates:
    ...
    using_default_module = lever_path is None
    ...
    try:
        ...
        if apply:
            ...
            # The codegen lever lands on a SEPARATE feat/autoloop-gen-* branch awaiting
            # a human merge — it is NOT live on the working branch — so the gap is
            # "attempted", not "closed". Record the branch + sha on the mechanism so the
            # provenance trail points at the pending review.
            led.add_gap(gap.with_status("attempted").with_mechanism(
                f"{hyp.mechanism} [codegen lever on {branch}@{sha}, pending merge]"))
            led.save()
        ...
        # Evict any spliced module the gate stack imported in THIS interpreter, so a
        # later plain import of generated.lever re-reads the restored identity file
        # rather than serving the stale spliced module from sys.modules. (Subprocess
        # G6 imports never touch the parent's sys.modules; this guards the in-process
        # flag-ON hook path.) Only for the real default module — never for a tmp file.
        if using_default_module:
            sys.modules.pop(_GENERATED_LEVER_MODULE, None)
