...
from perovskite_sim.models.device import DeviceStack, InterfaceDefect
...
def _canonical(value: Any) -> Any:
    if dataclasses.is_dataclass(value):
        ...
        if isinstance(value, DeviceStack):
            ...
            if value.interface_charge_closure == "off":
                # D4-E0 retains microscopic interface kinetics as provenance,
                # while every charge-off solver still executes the historical
                # resolved SRV tuple. Ignore the appended document in that
                # compatibility semantic hash. Charge-on stacks retain it in
                # full because occupancy, recombination and sheet charge must
                # be bound to one microscopic population.
                mapping["interface_defects"] = tuple(
                    (
                        dataclasses.replace(
                            defect,
                            microscopic_document=None,
                        )
                        if isinstance(defect, InterfaceDefect)
                        and defect.microscopic_document is not None
                        else defect
                    )
                    for defect in value.interface_defects
                )
        ...
        if (
            isinstance(value, InterfaceDefect)
            and value.microscopic_document is None
        ):
            # D4-E0 appends optional microscopic provenance. Its absence is the
            # exact historical InterfaceDefect shape and must not churn frozen
            # charge-off semantic hashes.
            mapping.pop("microscopic_document", None)
        if (
