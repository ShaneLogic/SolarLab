"""Content-addressed refinement adapter for the dual-mobile-ion DAE."""
...
from .dae_ion_refinement import (
    _finite_option,
    _integer_option,
...
def _boolean_option(options: dict[str, Any], name: str, default: bool) -> bool:
    ...
    if not isinstance(value, bool):
        raise ValueError(f"lane option {name!r} must be boolean")
    return value
...
def _execution_protocol(
    ...
    *,
    ...
    negative_ion_diffusion_m2_s: float,
    negative_ion_density_m3: float,
    negative_ion_site_limit_m3: float,
    ion_steric_diffusion_only: bool,
    ion_steric_shared_site: bool,
    base_time_steps: int,
    ...
    return {
        "backward_euler": {
            ...
            "time_coordinate": "physical_carrier_and_dual_ion_density",
        },
        "derived_negative_ion": {
            "diffusion_m2_s": negative_ion_diffusion_m2_s,
            "equilibrium_density_m3": negative_ion_density_m3,
            "site_limit_m3": negative_ion_site_limit_m3,
        },
        "matrix": {
        ...
        "schema_version": "dual-mobile-ion-dae-refinement-protocol-v1",
        "source_slice": {
        ...
        "topology": {
            ...
            "interfaces": "excluded",
            "ion_boundary": "blocking_zero_flux_each_species",
            "ion_coordinates": (
                "shared_site_three_state_softmax"
                if ion_steric_diffusion_only and ion_steric_shared_site
                else "independent_shifted_logits"
            ),
            "ion_reference_time_s": ion_reference_time_s,
            "ion_steric_diffusion_only": ion_steric_diffusion_only,
            "ion_steric_shared_site": ion_steric_shared_site,
            "mobile_ions": "positive_and_negative_unit_charge",
            "poisson_potential": "algebraic",
        },
    }


def _max_relative_species_difference(
    left: np.ndarray,
    right: np.ndarray,
    count: int,
    *,
    ...
    if species == "positive":
        segment = slice(2 * count, 3 * count)
    elif species == "negative":
        segment = slice(3 * count, 4 * count)
    else:
        raise ValueError("species must be 'positive' or 'negative'")
    left_ion = np.asarray(left, dtype=float)[..., segment]
    right_ion = np.asarray(right, dtype=float)[..., segment]
    if (
        ...
        raise ValueError(
            f"{species}-ion trajectories must be finite, positive, and matched"
        )
    return float(np.max(np.abs(left_ion - right_ion) / right_ion))


def run_dual_ion_dae_transient(
    lane: LaneDefinition,
    point: MatrixPoint,
    project_root: Path,
) -> CellMeasurement:
    """Execute one dual-mobile-ion DAE grid-by-time-step cell."""
    options = lane.options
    if _string_option(options, "config_loader", "standard") != "standard":
        raise ValueError("the dual-ion DAE lane requires config_loader='standard'")
    ...
    negative_ion_diffusion_m2_s = _finite_option(
        options,
        "negative_ion_diffusion_m2_s",
        3.2e-18,
    )
    negative_ion_density_m3 = _finite_option(
        options,
        "negative_ion_density_m3",
        1.6e25,
    )
    negative_ion_site_limit_m3 = _finite_option(
        options,
        "negative_ion_site_limit_m3",
        1.6e27,
    )
    ion_steric_diffusion_only = _boolean_option(
        options,
        "ion_steric_diffusion_only",
        True,
    )
    ion_steric_shared_site = _boolean_option(
        options,
        "ion_steric_shared_site",
        True,
    )
    if not ion_steric_diffusion_only or not ion_steric_shared_site:
        raise ValueError(
            "the registered dual-ion DAE lane requires diffusion-only shared-site steric physics"
        )
    ...
    max_ion_coordinate_update = _finite_option(
        options,
        "max_ion_coordinate_update",
        2.0,
    )
    ...
    if layer.params is None:
        raise ValueError("dual-ion source layer requires electrical parameters")
    dual_layer = replace(
        layer,
        params=replace(
            layer.params,
            D_ion_neg=negative_ion_diffusion_m2_s,
            P0_neg=negative_ion_density_m3,
            P_lim_neg=negative_ion_site_limit_m3,
        ),
    )
    stack = replace(
        source,
    ...
    grid = multilayer_grid([Layer(dual_layer.thickness, point.grid)], alpha=1.0)
    ...
    initial = build_dual_ion_consistent_initial_condition(
        model,
        residual_tolerance=residual_tolerance,
    )
    if not initial.certified:
        raise RuntimeError("dual-ion DAE initial condition is not certified")
    ...
    if not bool(mol.success) or np.asarray(mol.y).shape != (4 * grid.size, 2):
        raise RuntimeError(f"high-accuracy dual-ion MoL reference failed: {mol.message}")
    mol_terminal = StateVec.unpack(mol.y[:, -1], grid.size)
    if mol_terminal.P_neg is None or model.material.P_ion0_neg is None:
        raise RuntimeError("dual-ion MoL reference omitted negative-ion state")
    ...
    if dense_terminal.P_neg is None or structured_terminal.P_neg is None:
        raise RuntimeError("dual-ion DAE path omitted negative-ion state")
    ...
    terminal_positive_error = float(
        np.max(np.abs(structured_terminal.P - mol_terminal.P) / mol_terminal.P)
    )
    terminal_negative_error = float(
        np.max(
            np.abs(structured_terminal.P_neg - mol_terminal.P_neg)
            / mol_terminal.P_neg
        )
    )
    ...
    structured_dense_positive_difference = _max_relative_species_difference(
        structured.physical_states,
        dense.physical_states,
        grid.size,
        species="positive",
    )
    structured_dense_negative_difference = _max_relative_species_difference(
        structured.physical_states,
        dense.physical_states,
        grid.size,
        species="negative",
    ...
    if initial_state.P_neg is None:
        raise RuntimeError("dual-ion initial state omitted negative-ion species")
    initial_positive_inventory = dual_cell_integral(grid, initial_state.P)
    initial_negative_inventory = dual_cell_integral(grid, initial_state.P_neg)
    mol_positive_inventory = dual_cell_integral(grid, mol_terminal.P)
    mol_negative_inventory = dual_cell_integral(grid, mol_terminal.P_neg)
    positive_rate_scale = initial_positive_inventory / final_time_s
    negative_rate_scale = initial_negative_inventory / final_time_s
    maximum_positive_balance_relative = max(
        dense.max_positive_ion_balance_defect_m2_s,
        structured.max_positive_ion_balance_defect_m2_s,
    ) / positive_rate_scale
    maximum_negative_balance_relative = max(
        dense.max_negative_ion_balance_defect_m2_s,
        structured.max_negative_ion_balance_defect_m2_s,
    ) / negative_rate_scale
    maximum_positive_rhs_relative = max(
        dense.max_positive_ion_rhs_inventory_rate_m2_s,
        structured.max_positive_ion_rhs_inventory_rate_m2_s,
    ) / positive_rate_scale
    maximum_negative_rhs_relative = max(
        dense.max_negative_ion_rhs_inventory_rate_m2_s,
        structured.max_negative_ion_rhs_inventory_rate_m2_s,
    ) / negative_rate_scale
    maximum_positive_inventory_drift = max(
        dense.max_relative_positive_ion_inventory_drift,
        structured.max_relative_positive_ion_inventory_drift,
        abs(mol_positive_inventory - initial_positive_inventory)
        / initial_positive_inventory,
    )
    maximum_negative_inventory_drift = max(
        dense.max_relative_negative_ion_inventory_drift,
        structured.max_relative_negative_ion_inventory_drift,
        abs(mol_negative_inventory - initial_negative_inventory)
        / initial_negative_inventory,
    )
    positive_motion = float(
        np.max(np.abs(structured_terminal.P - initial_state.P) / initial_state.P)
    )
    negative_motion = float(
        np.max(
            np.abs(structured_terminal.P_neg - initial_state.P_neg)
            / initial_state.P_neg
        )
    )
    ...
    if dense_work <= 0:
        raise RuntimeError("dense dual-ion DAE reference reported no residual work")
    ...
    topology_verified = bool(
        ...
        and model.material.has_dual_ions
        and model.layout.shared_site
        and np.all(model.material.D_ion_node > 0.0)
        and model.material.D_ion_neg_node is not None
        and np.all(model.material.D_ion_neg_node > 0.0)
    )
    count = grid.size
    trajectory_states = (dense.physical_states, structured.physical_states)
    site_occupancy_admissible = all(
        np.all(states[:, 2 * count : 3 * count] > 0.0)
        and np.all(states[:, 3 * count : 4 * count] > 0.0)
        and np.all(
            states[:, 2 * count : 3 * count]
            + states[:, 3 * count : 4 * count]
            < model.material.P_lim_node
        )
        for states in trajectory_states
    ) and bool(
        np.all(mol_terminal.P > 0.0)
        and np.all(mol_terminal.P_neg > 0.0)
        and np.all(
            mol_terminal.P + mol_terminal.P_neg < model.material.P_lim_node
        )
    )
    vacancy_candidates = [
        dense.minimum_site_vacancy_fraction,
        structured.minimum_site_vacancy_fraction,
        float(
            np.min(
                1.0
                - (mol_terminal.P + mol_terminal.P_neg)
                / model.material.P_lim_node
            )
        ),
    ]
    minimum_vacancy_fraction = min(vacancy_candidates)
    ...
    protocol = _execution_protocol(
        ...
        carrier_reference_time_s=carrier_reference_time_s,
        ion_reference_time_s=ion_reference_time_s,
        negative_ion_diffusion_m2_s=negative_ion_diffusion_m2_s,
        negative_ion_density_m3=negative_ion_density_m3,
        negative_ion_site_limit_m3=negative_ion_site_limit_m3,
        ion_steric_diffusion_only=ion_steric_diffusion_only,
        ion_steric_shared_site=ion_steric_shared_site,
        base_time_steps=base_time_steps,
        reference_grid_intervals=reference_grid_intervals,
    ...
    return CellMeasurement.from_mapping(
        {
            "observables": {
                "structured_dense_terminal_log_density_difference": max(
                    float(
                        np.max(
                            np.abs(np.log(structured_terminal.n / dense_terminal.n))
                        )
                    ),
                    float(
                        np.max(
                            np.abs(np.log(structured_terminal.p / dense_terminal.p))
                        )
                    ),
                ),
                "structured_dense_terminal_positive_ion_relative_difference": (
                    float(
                        np.max(
                ...
                "structured_dense_terminal_negative_ion_relative_difference": (
                    float(
                        np.max(
                            np.abs(
                                structured_terminal.P_neg - dense_terminal.P_neg
                            )
                            / dense_terminal.P_neg
                        )
                    )
                ),
                ...
                "terminal_positive_ion_relative_error": terminal_positive_error,
                "terminal_negative_ion_relative_error": terminal_negative_error,
            ...
            "quality": {
                ...
                "dual_ion_topology_verified": float(topology_verified),
                ...
                "max_negative_ion_balance_relative": maximum_negative_balance_relative,
                "max_negative_ion_inventory_relative_drift": (
                    maximum_negative_inventory_drift
                ),
                "max_negative_ion_rhs_inventory_relative": (
                    maximum_negative_rhs_relative
                ),
                ...
                "max_normalized_negative_ion_residual": max(
                    dense.max_normalized_negative_ion_residual,
                    structured.max_normalized_negative_ion_residual,
                ),
                ...
                "max_positive_ion_balance_relative": maximum_positive_balance_relative,
                "max_positive_ion_inventory_relative_drift": (
                    maximum_positive_inventory_drift
                ),
                "max_positive_ion_rhs_inventory_relative": (
                    maximum_positive_rhs_relative
                ),
                "max_structured_dense_log_density_difference": (
                    structured_dense_log_difference
                ),
                "max_structured_dense_negative_ion_relative_difference": (
                    structured_dense_negative_difference
                ),
                "max_structured_dense_positive_ion_relative_difference": (
                    structured_dense_positive_difference
                ),
                ...
                "max_terminal_negative_ion_relative_error": terminal_negative_error,
                "max_terminal_positive_ion_relative_error": terminal_positive_error,
                "max_terminal_potential_error_V": terminal_potential_error,
                "minimum_negative_ion_relative_motion": negative_motion,
                "minimum_positive_ion_relative_motion": positive_motion,
                "minimum_shared_site_vacancy_fraction": minimum_vacancy_fraction,
            ...
            "units": {
                ...
                "dual_ion_topology_verified": "1",
                "max_electron_balance_defect_A_m2": "A m-2",
                "max_hole_balance_defect_A_m2": "A m-2",
                "max_negative_ion_balance_relative": "1",
                "max_negative_ion_inventory_relative_drift": "1",
                "max_negative_ion_rhs_inventory_relative": "1",
                "max_normalized_algebraic_residual": "1",
                "max_normalized_carrier_residual": "1",
                "max_normalized_negative_ion_residual": "1",
                ...
                "max_structured_dense_negative_ion_relative_difference": "1",
                ...
                "max_terminal_negative_ion_relative_error": "1",
                "max_terminal_positive_ion_relative_error": "1",
                "max_terminal_potential_error_V": "V",
                "minimum_negative_ion_relative_motion": "1",
                "minimum_positive_ion_relative_motion": "1",
                "minimum_shared_site_vacancy_fraction": "1",
                ...
                "structured_dense_terminal_negative_ion_relative_difference": "1",
                ...
                "terminal_negative_ion_relative_error": "1",
            ...
            "metadata": {
                **_protocol_metadata(protocol),
                "actual": {
                    ...
                    "initial_negative_ion_inventory_m2": (
                        initial_negative_inventory
                    ),
                    "initial_positive_ion_inventory_m2": (
                        initial_positive_inventory
                    ),
...
__all__ = ["run_dual_ion_dae_transient"]
