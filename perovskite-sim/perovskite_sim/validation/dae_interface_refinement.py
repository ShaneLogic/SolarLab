"""Content-addressed refinement adapter for the algebraic-interface DAE."""
...
def _execution_protocol(
    ...
    *,
    ...
    return {
        ...
        "schema_version": "algebraic-interface-dae-refinement-protocol-v1",
        "source_slice": {
        ...
        "topology": {
            ...
            "poisson_potential": "algebraic_charge_off",
            "two_sided_trace_geometry": "excluded",
        },
    }


def _terminal_carriers(
    state: np.ndarray,
    node_count: int,
...
def _max_relative_interface_difference(
    left: np.ndarray,
    right: np.ndarray,
) -> float:
    ...
    if (
        left_state.shape != right_state.shape
        or not np.all(np.isfinite(left_state))
        or not np.all(np.isfinite(right_state))
        or np.any(left_state <= 0.0)
        or np.any(right_state <= 0.0)
    ):
        raise ValueError(
            "interface trajectories must be finite, positive, and shape matched"
        )
    return float(np.max(np.abs(left_state - right_state) / right_state))


def run_algebraic_interface_dae_transient(
    lane: LaneDefinition,
    point: MatrixPoint,
    project_root: Path,
) -> CellMeasurement:
    """Execute one algebraic-interface DAE grid-by-time-step cell."""
    options = lane.options
    if _string_option(options, "config_loader", "standard") != "standard":
        raise ValueError(
            "the algebraic-interface DAE lane requires config_loader='standard'"
        )
    ...
    final_time_s = _finite_option(options, "final_time_s", 1.0e-8)
    ...
    mol_rtol = _finite_option(options, "mol_rtol", 1.0e-8)
    mol_atol_m3 = _finite_option(options, "mol_atol_m3", 1.0e4)
    ...
    if not initial.certified:
        raise RuntimeError(
            "algebraic-interface DAE consistent initial condition is not certified"
        )
    ...
    mol_state = StateVec.unpack(mol.y[:, -1], grid.size)
    ...
    initial_tangent = build_algebraic_interface_structured_backward_euler_jacobian(
        model,
        initial.coordinate,
    ...
    minimum_projection_margin = min(
        item.minimum_projection_occupation_margin for item in local_evidence
    )
    minimum_cross_margin = min(
        item.minimum_cross_occupation_margin for item in local_evidence
    )
    minimum_srh_margin = min(
        item.minimum_srh_occupancy_margin for item in local_evidence
    )
    minimum_density_margin = min(
        item.minimum_interface_density_margin_m3 for item in local_evidence
    )
    minimum_dos_margin = min(
        item.minimum_interface_dos_margin_m3 for item in local_evidence
    )
    ...
    maximum_structured_condition = max(
        (
            report.max_scaled_jacobian_condition
            for report in structured.step_reports
        ),
        default=0.0,
    )
    if dense.total_residual_evaluations <= 0:
        raise RuntimeError("dense interface DAE reference reported no residual work")
    ...
    terminal_interface_bounded = bool(
        np.all(structured.interface_states_m3 > 0.0)
        and np.all(
            structured.interface_states_m3
            < model.layout.interface_capacity_m3[np.newaxis, :]
        )
        and np.all(dense.interface_states_m3 > 0.0)
        and np.all(
            dense.interface_states_m3
            < model.layout.interface_capacity_m3[np.newaxis, :]
        )
    ...
    return CellMeasurement.from_mapping(
        {
            "observables": {
                ...
                "structured_dense_terminal_potential_difference_V": float(
                    np.max(
                        np.abs(
                            structured.potentials_V[-1]
                            - dense.potentials_V[-1]
                        )
                    )
                ),
            ...
            "quality": {
                "algebraic_interface_topology_verified": float(topology_verified),
                ...
                "clamp_inactive_slice_verified": float(
                    ...
                    and minimum_density_margin > 0.0
                    and minimum_dos_margin > 0.0
                ),
                ...
                "terminal_interface_states_bounded": float(
                    terminal_interface_bounded
                ),
            },
            "units": {
                "algebraic_interface_topology_verified": "1",
            ...
            "metadata": {
                **_protocol_metadata(protocol),
                "actual": {
                    ...
                    "minimum_interface_density_margin_m3": minimum_density_margin,
                    "minimum_interface_dos_margin_m3": minimum_dos_margin,
                    "minimum_projection_occupation_margin": (
...
__all__ = ["run_algebraic_interface_dae_transient"]
