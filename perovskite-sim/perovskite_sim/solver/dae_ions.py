"""Research-only semi-explicit DAE for one positive mobile-ion species.

This is the second DAE topology checkpoint.  It deliberately supports only a
single electrical layer with ohmic carrier contacts, one positive ion species
active at every node, blocking ion boundaries, and no physical/interface-state
...
class SingleIonDAELayout:
    """Coordinate layout for ``(log n, log p, logit theta_pos, phi)``."""
...
@dataclass(frozen=True, slots=True)
class SingleIonDAEResidualReport:
    """Differential, algebraic, and blocking-inventory residual evidence."""
...
@dataclass(frozen=True, slots=True)
class SingleIonDAEConsistentInitialCondition:
    """Reproducible state/derivative pair satisfying every single-ion row."""

    coordinate: np.ndarray
    derivative: np.ndarray
    physical_state: np.ndarray
    potential_V: np.ndarray
    report: SingleIonDAEResidualReport
    certified: bool
    state_sha256: str


@dataclass(frozen=True, slots=True)
class SinglePositiveIonDAE:
    """Narrow DAE retaining positive-ion and Poisson coordinates explicitly."""

    grid_m: np.ndarray
    stack: DeviceStack
    material: MaterialArrays
    layout: SingleIonDAELayout
    V_app_V: float
    illuminated: bool
    ...
    def physical_fields(
        ...
        if value.shape != (self.layout.size,) or not np.all(np.isfinite(value)):
            raise ValueError("single-ion DAE coordinate must be finite and layout-sized")
        layout = self.layout
        ...
        if not np.all(np.isfinite(n)) or not np.all(np.isfinite(p)):
            raise ValueError("single-ion DAE log-density coordinate overflowed")
        if np.any(theta <= 0.0) or np.any(theta >= 1.0):
            raise ValueError("single-ion DAE logit coordinate saturated")
        return n, p, positive_ion, phi

    def positive_ion_coordinate_derivative_m3(
        self,
        coordinate: np.ndarray,
    ) -> np.ndarray:
        _n, _p, positive_ion, _phi = self.physical_fields(coordinate)
        theta = positive_ion / self.layout.positive_ion_site_limit_m3
        return positive_ion * (1.0 - theta)

    def packed_physical_state(self, coordinate: np.ndarray) -> np.ndarray:
        n, p, positive_ion, _phi = self.physical_fields(coordinate)
        return StateVec.pack(n, p, positive_ion)

    def compatible_derivative(self, coordinate: np.ndarray) -> np.ndarray:
        """Return qdot that satisfies all differential rows at one coordinate."""
        layout = self.layout
        n, p, _positive_ion, phi = self.physical_fields(coordinate)
        rhs = StateVec.unpack(
            assemble_rhs(
                0.0,
        ...
        ion_coordinate_derivative = self.positive_ion_coordinate_derivative_m3(
            coordinate
        )
        derivative[layout.positive_ion_slice] = rhs.P / ion_coordinate_derivative
        if not np.all(np.isfinite(derivative)):
            raise ValueError("single-ion compatible derivative is non-finite")
        return derivative
    ...
    def residual_report(
        ...
        coordinate: np.ndarray,
        derivative: np.ndarray,
    ) -> SingleIonDAEResidualReport:
        layout = self.layout
        rate = np.asarray(derivative, dtype=float)
        if rate.shape != (layout.size,) or not np.all(np.isfinite(rate)):
            raise ValueError("single-ion DAE derivative must be layout-sized and finite")
        n, p, positive_ion, phi = self.physical_fields(coordinate)
        rhs = StateVec.unpack(
            assemble_rhs(
                0.0,
        ...
        positive_ion_rows = normalized[layout.positive_ion_slice]
        ...
        return SingleIonDAEResidualReport(
            normalized_residual=_readonly_f64(
                normalized,
                shape=(layout.size,),
                name="single-ion normalized residual",
            ),
            ...
            max_normalized_positive_ion_residual=float(
                np.max(np.abs(positive_ion_rows), initial=0.0)
            ),
            max_normalized_differential_residual=float(
    ...
    def derivative_jacobian_diagonal(self, coordinate: np.ndarray) -> np.ndarray:
        layout = self.layout
        n, p, _positive_ion, _phi = self.physical_fields(coordinate)
    ...
    def algebraic_state_jacobian(self, coordinate: np.ndarray) -> np.ndarray:
        n, p, _positive_ion, _phi = self.physical_fields(coordinate)
        positive_ion_derivative = self.positive_ion_coordinate_derivative_m3(
            coordinate
        )
        layout = self.layout
        ...
        for local, node in enumerate(range(1, count - 1)):
            ...
            result[row, 2 * count + node] = (
                Q * positive_ion_derivative[node] * widths[local] / scale
            )
...
def _validate_single_positive_ion_capability(
    stack: DeviceStack,
    material: MaterialArrays,
    ...
    if (
        ...
        violations.append("positive-ion reference must lie strictly inside site limits")
    if violations:
...
def build_single_positive_ion_dae(
    grid_m: np.ndarray,
    stack: DeviceStack,
    ...
    *,
...
) -> SinglePositiveIonDAE:
    """Build the parked single-positive-ion DAE capability."""
    ...
    state = _validate_single_positive_ion_capability(
        stack,
        mat,
        reference_state,
        grid.size,
    ...
    theta = positive_ion / mat.P_lim_node
    logit_reference = np.log(theta) - np.log1p(-theta)
    ...
    differential_mask = np.zeros(4 * grid.size, dtype=bool)
...
def project_single_ion_algebraic_state(
    ...
    n, p, positive_ion, _phi = model.physical_fields(result)
    rho = Q * (
        p
        - n
        + positive_ion
...
def build_single_ion_consistent_initial_condition(
    model: SinglePositiveIonDAE,
    *,
    residual_tolerance: float = 1.0e-10,
) -> SingleIonDAEConsistentInitialCondition:
    ...
    coordinate = project_single_ion_algebraic_state(
        model,
    ...
    _n, _p, _positive_ion, potential = model.physical_fields(coordinate)
    coordinate_ro = _readonly_f64(
        coordinate,
        shape=(model.layout.size,),
        name="single-ion consistent coordinate",
    )
    derivative_ro = _readonly_f64(
        derivative,
        shape=(model.layout.size,),
        name="single-ion consistent derivative",
    )
    physical_ro = _readonly_f64(
        physical,
        shape=(3 * model.layout.node_count,),
        name="single-ion consistent physical state",
    )
    potential_ro = _readonly_f64(
        potential,
        shape=(model.layout.node_count,),
        name="single-ion consistent potential",
    )
    return SingleIonDAEConsistentInitialCondition(
        ...
        state_sha256=_state_sha256(
            "single-positive-ion-dae-initial-v1",
            model.grid_m,
            coordinate_ro,
            derivative_ro,
            physical_ro,
...
def finite_difference_single_ion_state_jacobian(
    model: SinglePositiveIonDAE,
    coordinate: np.ndarray,
    derivative: np.ndarray,
    *,
    ...
    """Independent central reference for single-ion ``dF/dq``."""
    ...
    if value.shape != (model.layout.size,):
        raise ValueError("coordinate does not match the single-ion layout")
...
def finite_difference_single_ion_derivative_jacobian(
    model: SinglePositiveIonDAE,
    coordinate: np.ndarray,
    derivative: np.ndarray,
    *,
    ...
    """Independent central reference for single-ion ``dF/d(qdot)``."""
    if not np.isfinite(relative_step) or relative_step <= 0.0:
        raise ValueError("relative_step must be finite and positive")
    rate = np.asarray(derivative, dtype=float)
    if rate.shape != (model.layout.size,):
        raise ValueError("derivative does not match the single-ion layout")
    layout = model.layout
    ion_derivative = model.positive_ion_coordinate_derivative_m3(coordinate)
    ...
    for column in range(layout.size):
        if column < layout.node_count:
            ...
            rate_scale = (
                layout.hole_rate_scale_m3_s[local]
                / layout.hole_reference_m3[local]
            )
        elif column < 3 * layout.node_count:
            local = column - 2 * layout.node_count
            rate_scale = (
                layout.positive_ion_rate_scale_m3_s[local]
                / ion_derivative[local]
            )
        else:
            rate_scale = 1.0
