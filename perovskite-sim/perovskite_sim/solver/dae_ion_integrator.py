"""Conservative time reference for the single-positive-ion research DAE."""
...
def _trajectory_sha256(
    ...
    digest = hashlib.sha256(b"single-positive-ion-dae-be-v1")
    for value in (
        model.grid_m,
        time_s,
        coordinates,
...
def _positive_ion_density_difference_m3(
    ...
    layout = model.layout
    _n, _p, positive_ion, _phi = model.physical_fields(coordinate)
    _old_n, _old_p, old_positive_ion, _old_phi = model.physical_fields(
        previous_coordinate
    )
    ...
    if not np.all(np.isfinite(difference)):
        raise ValueError("single-ion logistic density difference is non-finite")
    return difference


@dataclass(frozen=True, slots=True)
class SingleIonDAETimeStepReport:
    """Nonlinear, local-balance, and blocking-inventory evidence for one step."""
    ...
    residual_report: SingleIonDAEResidualReport
...
@dataclass(frozen=True, slots=True)
class SingleIonDAETransientResult:
    """Certified trajectory from the single-positive-ion BE reference."""

    time_s: np.ndarray
    coordinates: np.ndarray
    physical_states: np.ndarray
    potentials_V: np.ndarray
    step_reports: tuple[SingleIonDAETimeStepReport, ...]
    success: bool
    method: str
    jacobian_mode: str
...
def single_ion_backward_euler_derivative(
    model: SinglePositiveIonDAE,
    coordinate: np.ndarray,
    previous_coordinate: np.ndarray,
    dt_s: float,
    ...
    if value.shape != (layout.size,) or previous.shape != (layout.size,):
        raise ValueError("BE coordinates must match the single-ion DAE layout")
    _n, _p, positive_ion, _phi = model.physical_fields(value)
    ...
    if not np.all(np.isfinite(derivative)):
        raise ValueError("single-ion backward-Euler coordinate rate is non-finite")
    return derivative


def single_ion_backward_euler_derivative_chain(
    model: SinglePositiveIonDAE,
    coordinate: np.ndarray,
    previous_coordinate: np.ndarray,
    dt_s: float,
) -> np.ndarray:
    """Return exact diagonal ``d(qdot_BE)/dq_new`` for all differential rows."""
    if not np.isfinite(dt_s) or dt_s <= 0.0:
        raise ValueError("dt_s must be finite and positive")
    layout = model.layout
    value = np.asarray(coordinate, dtype=float)
    ...
    diagonal[layout.positive_ion_slice] = (
        1.0
        - density_difference * (1.0 - 2.0 * theta) / ion_storage
    ) / dt_s
    if not np.all(np.isfinite(diagonal)):
        raise ValueError("single-ion backward-Euler derivative chain is non-finite")
    return diagonal


def finite_difference_single_ion_backward_euler_jacobian(
    model: SinglePositiveIonDAE,
    coordinate: np.ndarray,
    previous_coordinate: np.ndarray,
    dt_s: float,
    *,
    ...
    for column in range(layout.size):
        ...
        plus[column] += step
        minus[column] -= step
        plus_rate = single_ion_backward_euler_derivative(
            model, plus, previous, dt_s
        )
        minus_rate = single_ion_backward_euler_derivative(
            model, minus, previous, dt_s
        )
        result[:, column] = (
...
def _validate_initial_condition(
    model: SinglePositiveIonDAE,
    initial: SingleIonDAEConsistentInitialCondition,
    ...
    _n, _p, _positive_ion, expected_potential = model.physical_fields(
        initial.coordinate
    )
    if not initial.certified:
        raise ValueError("initial single-ion DAE condition is not certified")
    if report.max_normalized_residual > residual_tolerance:
        raise ValueError(
            "initial single-ion DAE condition exceeds the residual tolerance"
        )
    if (
        ...
        raise ValueError("initial single-ion DAE condition does not belong to model")
...
def run_single_ion_backward_euler_reference(
    model: SinglePositiveIonDAE,
    time_s: np.ndarray,
    *,
    initial: SingleIonDAEConsistentInitialCondition | None = None,
...
) -> SingleIonDAETransientResult:
    """Integrate the narrow single-ion DAE with conservative backward Euler."""
    ...
    reports: list[SingleIonDAETimeStepReport] = []
    ...
    for step_index, (time_start, time_end) in enumerate(
        ...
        worst_condition = 0.0
        try:
            coordinate = project_single_ion_algebraic_state(model, prediction)
        except ValueError as exc:
            raise DAEIntegrationError(
                f"single-ion algebraic predictor projection failed: {exc}",
        ...
        ) -> tuple[np.ndarray, SingleIonDAEResidualReport]:
        ...
        while residual_norm > residual_tolerance:
            if nonlinear_iterations >= max_newton_iterations:
                raise DAEIntegrationError(
                    "single-ion Newton iterations exhausted",
                    step_index=step_index,
                    time_s=float(time_end),
            ...
            try:
                if jacobian_mode == "dense_central":
                    jacobian = finite_difference_single_ion_backward_euler_jacobian(
                        model,
                        coordinate,
                        previous,
                        dt_s,
                ...
                raise DAEIntegrationError(
                    f"single-ion Jacobian solve failed: {exc}",
                    step_index=step_index,
                    time_s=float(time_end),
                    residual_norm=residual_norm,
                ) from exc
            ...
            if not accepted:
                raise DAEIntegrationError(
                    "single-ion Newton line search stalled",
                    step_index=step_index,
                    time_s=float(time_end),
                    residual_norm=residual_norm,
                )
        ...
        _n, _p, positive_ion, potential = model.physical_fields(coordinate)
    ...
    return SingleIonDAETransientResult(
        ...
        success=True,
        method=(
            "physical-density-backward-euler/single-ion-dense-hybrid-newton-v1"
            if jacobian_mode == "dense_central"
            else "physical-density-backward-euler/single-ion-sparse-analytic-newton-v1"
        ),
        ...
        max_normalized_carrier_residual=max(
            (item.residual_report.max_normalized_carrier_residual for item in report_tuple),
            default=0.0,
        ),
