...
`qualification_scope(record, state_sha256=..., domain=...)` returns exactly:
...
step records, and independently reviewed budget, turnover or prerequisite
evidence. A producer's `review_status: approved`, its own manifest, or the
...
- `evidence_id`, exact `scope`, `classification` (`bounded`, `estimate_only`,
  or `unknown`), `units: A/m2`, and the exact `amplitude_V`;
...
by zero. An empirical refinement estimate remains `estimate_only` even if
its provenance is verified. No real-device budget is approved by this code.
...
requires the frozen full 0--100 s observation grid. Analytic fixtures may use
...
Error items require `classification: bounded`. Bare pass flags cannot supply
