    ...
    <div class="doc-body">
      ...
      <table class="param-table">
        ...
        <tbody>
          ...
          <tr><td><i>P</i><sub>lim,−</sub></td><td>Site density (anions / −)</td><td>m⁻³</td><td>Site limit for the second species, and always its own — sharing a sublattice changes the numerator of the occupancy, never this denominator. Defaults to 10<sup>30</sup>, which switches the anion's crowding off (<i>θ</i> ≈ 0) even with sharing on, so a shared-sublattice stack should set it equal to <i>P</i><sub>lim</sub>. Writing 0 would not remove the limit; it would divide by it.</td></tr>
          <tr><td><i>P</i><sub>0,−</sub></td><td>Background ion density (anions / −)</td><td>m⁻³</td><td>Reference density for the negative species in the Poisson term. Setting it equal to <i>P</i><sub>0</sub> leaves the ions net-neutral at rest, so the equilibrium electrostatics match the single-ion case.</td></tr>
          <tr><td><i>E</i><sub>a,ion</sub></td><td>Ion activation energy</td><td>eV</td><td>Arrhenius exponent for <i>D</i><sub>ion</sub>(<i>T</i>) = <i>D</i><sub>ion,300</sub>·exp[−<i>E</i><sub>a</sub>/<i>k</i><sub>B</sub>·(1/<i>T</i> − 1/300)]. Default 0.58. A single value covers both species — the same <i>E</i><sub>a,ion</sub> scales <i>D</i><sub>ion</sub> and <i>D</i><sub>ion,−</sub>; there is no per-species variant. Explicit 0 removes the temperature dependence. Active in <b>Fast</b> and <b>Full</b>; Legacy pins <i>T</i> to 300 K.</td></tr>
          <tr><td><code>ion_steric_shared_site</code></td><td>Shared-sublattice flag</td><td>bool</td><td>Device-level. When <code>true</code> (the default) each species' crowding potential −ln(1−<i>θ</i>) uses the <i>total</i> occupancy <i>θ</i> = (<i>P</i><sub>+</sub>+<i>P</i><sub>−</sub>)/<i>P</i><sub>lim</sub> of that species, so the two compete for one reservoir; when <code>false</code> each crowds only against itself. Applies only under the diffusion-only steric form.</td></tr>
