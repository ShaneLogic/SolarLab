...
**Tiered simulation modes (Phase 5 — Apr 2026).** Every Phase 1–3 physics upgrade is gated on a flag in `SimulationMode` (defined in `perovskite_sim/models/mode.py`). The three named tiers — `LEGACY`, `FAST`, `FULL` — bundle the eight flags into useful combinations so that callers can pick a fidelity / speed trade-off without editing individual flags. The tier acts as a **ceiling**: turning a flag on only enables the corresponding hook if the config actually provides the parameter the hook needs (TMM needs `optical_material`, TE needs non-zero `chi` and `Eg` with a real interface offset, selective contacts need at least one `S_*`, field-mobility needs `v_sat_{n,p}` or `pf_gamma_{n,p}`, etc.). This "enable-if-configured" pattern means the tier strings are safe to set on every preset — flags are silent when inactive rather than forcing physics that the stack cannot support. Flag matrix: **LEGACY** turns everything off for IonMonger reproducibility. **FAST** turns on every *build-once* upgrade (TE, TMM, dual ions, trap profile, T-scaling, photon recycling) but keeps the two *per-RHS* upgrades off — field-dependent mobility (3.2) and selective contacts (3.3) both break the MaterialArrays build-once invariant, so FAST tier skips them. **FULL** turns everything on, including the per-RHS hooks. A YAML preset can pin a tier with a top-level `device.mode: legacy|fast|full`; `DeviceStack.mode` defaults to `"full"` so any config that does not pin a mode gets the full physics set. `resolve_mode` (in `models/mode.py`) is idempotent on `SimulationMode` instances, parses strings case-insensitively, and returns `FULL` for `None`. The backend renders the resolved tier into the SSE result payload via `_describe_active_physics`, which produces strings like `"FULL  band offsets · TE · TMM · dual ions · trap profile · T-scaling · photon recycling · μ(E) · Robin contacts"` — the per-RHS fragments only appear when the corresponding flag is on, so FAST and LEGACY get a shorter label. The tier system is invariant-tested in `tests/unit/models/test_mode.py` (flag monotonicity: `LEGACY ⊆ FAST ⊆ FULL`), and end-to-end in `tests/regression/test_tier_regression.py` (LEGACY reproduces FAST/FULL on a plain preset; FAST ignores per-RHS opt-ins even when the config provides them; LEGACY disables TE even with band offsets configured; YAML `mode:` round-trips through the full loader).
