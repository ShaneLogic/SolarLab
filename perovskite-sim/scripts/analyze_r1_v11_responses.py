...
from scripts import run_r1_v11_cases as cases_runner
...
SCHEMAS = {"collect-dc": "R1V11DCBaselineRequestV1", "analyze": "R1V11ResponseAnalysisRequestV1"}
...
def load_request(path, expected, output, command):
    ...
    if not isinstance(request, dict) or set(request) != required or request.get("schema") != SCHEMAS[command]:
        raise ValueError("V11 response request schema or fields differ")
    ...
    if (not isinstance(selected, list) or len(selected) != len(expected_ids)
        ...
        raise ValueError("V11 D2 requires the exact distinct source-bound case inventory")
    return request, raw
...
def preparation_identity(prepared):
    ...
    return {"schema": "R1V11PairPreparationIdentityV1", "sha256": digest(content),
...
def load_cases(request, actual_source, output):
    ...
    for item in request["cases"]:
        ...
        if (summary.get("schema") != "R1V11CaseRunV1" or summary.get("case_id") != name
            ...
            raise ValueError("V11 D2 case, source or representation binding differs: " + name)
...
def collect_dc(request, cases, output, actual_source, context):
    ...
    summary = {"schema": "R1V11DCBaselineCollectionV1", "source_commit": request["source_commit"],
...
def load_baselines(request, actual_source):
    ...
    if (summary.get("schema") != "R1V11DCBaselineCollectionV1" or summary.get("actual_source") != actual_source
            or any(summary.get(key) != request[key] for key in ("source_commit", "source_content_sha256"))
        ...
        raise ValueError("V11 DC source, completion or case inventory differs")
    baselines = {}
    for item in summary["entries"]:
        ...
        if (dc.get("schema") != "R1ControlledDCResponseV2" or dc.get("representation") != "float64-pair-v1"
            ...
            raise ValueError("V11 DC baseline is not bound to exact preparation and full physical replay")
...
def analyze(request, cases, baselines, actual_source):
    ...
    return {"schema": "R1V11DResponseAnalysisV1", "source_commit": request["source_commit"],
...
def run(args):
    if not (sys.flags.isolated and sys.flags.no_site) or any(os.environ.get(key) != "1" for key in THREADS):
        raise ValueError("V11 response tools require -I -S and all five thread limits equal to 1")
    output = args.output.resolve()
    request, raw = load_request(args.request_file, args.request_sha256, output, args.command)
    if output.exists():
        raise FileExistsError("V11 response output already exists")
    ...
    with threadpool_limits(1):
        ...
        if not pools or any(item["num_threads"] != 1 for item in pools) or np.__version__ != "2.1.3" or scipy.__version__ != "1.15.3":
            raise ValueError("V11 response runtime differs from its frozen dependency lineage")
    ...
    if sha(args.request_file) != args.request_sha256:
        raise ValueError("external V11 response request changed during execution")
