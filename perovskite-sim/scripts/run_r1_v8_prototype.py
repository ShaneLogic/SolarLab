"""Run the externally frozen V8 N16 prefix with both-side precision evidence.
...
SCHEMA = "R1V8PrototypeContractV1"
SCOPE = "development_P1_completion_not_production_or_R1_2_qualification"
...
def validation_contract():
    """Fixed V8 responsibilities and pre-fault choices from the approved S0 freeze."""
    return {'schema': 'R1V8ValidationContractV1',
     'path_calibration_sha256': '67ed0d0938ca4ee5e43e6011a40111bfd59cf357783c8c31ae592dc7f3e0a113',
     'ordinary_state': {'row': 342, 'substeps': 4, 'time_s': 2.5e-10},
     'late_state': {'row': 791, 'substeps': 4, 'time_s': 1.9663570500354033},
     'path_state': {'row': 770, 'substeps': 4, 'time_s': 0.7173200981104755},
     'shared_faults': {'diffusion_multiplier': 1.01,
                       'thermal_voltage_multiplier': 1.01,
                       'drift_sign': -1,
                       'omitted_ion': True,
                       'face_sign': 'first_active_nonzero_face'},
     'boundary_leak_m2_s': 1e-06,
     'healthy_potential_perturbation_V': [-1e-23, 1e-23],
     'path_potential_perturbation_V': [-1e-13, 1e-13],
     'path_node': 7,
     'replay_mutation': {'field': 'precision_phi_V_lo',
                         'index': 1,
                         'increment_V': 1e-23,
                         'prepared': True,
                         'trajectory_row': 1,
                         'reseal_duplicates_and_hashes': True},
     'legacy_wrong_previous': {'target_row': 2, 'correct_row': 1, 'replacement_row': 0, 'substeps': 1},
     'both_actual_sides_required': True,
     'initial_upstream_arithmetic_required': True,
     'actual_resealed_replay_required': True,
     'all_final_evidence_same_source': True}
...
def make_contract(historical_request):
    ...
    return {
        "schema": SCHEMA, "scope": SCOPE, "validation_contract": validation_contract(), "case_id": "D_N16_F0p1_T1_To2p154434690031884s",
        ...
        "precision_requirements": {
            ...
            "numeric_budget_freeze_complete": True,
            "both_actual_sides_required": True, "initial_arithmetic_required": True,
...
def load_contract(path, digest, output):
    ...
    if contract.get("schema") != SCHEMA or contract.get("scope") != SCOPE:
        raise ValueError("unsupported V8 prototype contract")
    ...
    if contract.get("validation_contract") != validation_contract():
        raise ValueError("V8 validation and mutation contract changed")
    precision = contract.get("precision_requirements", {})
    if (precision.get("numeric_budget_freeze_complete") is not True
            or precision.get("both_actual_sides_required") is not True
            or precision.get("initial_arithmetic_required") is not True):
        raise ValueError("V8 requires the frozen precision budget and both actual sides")
    return contract, raw
...
def cost_comparison(current, baseline, contract_digest):
    ...
    reasons = []
    if (baseline.get("schema") != "R1V8PrototypeRunV1" or baseline.get("mode") != "baseline"
    ...
    if any(not current.get(key) or baseline.get(key) != current.get(key)
           for key in ("source_commit", "source_content_sha256")):
        reasons.append("source_identity_differs")
    ratios = {}
...
def run(args):
    ...
    case = contract["case"]
    summary = {"schema": "R1V8PrototypeRunV1", "scope": SCOPE, "mode": args.mode,
    ...
    def source_guard():
        if source_snapshot(args.expected_commit) != initial_source or sha(args.request_file) != args.request_sha256:
            raise ValueError("source or external request changed during V8 prototype execution")
    ...
    try:
        ...
        with threadpool_limits(limits=1), manager:
            ...
            from scripts.r1_v8_timing import ExclusiveTimer, instrument
            timer = ExclusiveTimer()
            measured = instrument(run_trajectory, timer)
            summary.update(measured(output, case, args.mode, api, source_guard))
            summary["phase_timing"] = timer.report(summary["cost"]["elapsed_s"])
            write(output / "PhaseTimingV1.json", summary["phase_timing"])
    ...
    if args.mode == "compensated":
        rows_path = output / "AcceptedStepsV1.jsonl"
        rows_for_coverage = [json.loads(line) for line in rows_path.read_text().splitlines()]
        missing_sides = [index for index, row in enumerate(rows_for_coverage)
                         if not isinstance(row.get("physics_reconstruction", {}).get("eliminated_precision"), dict)]
        summary["eliminated_precision_coverage"] = {
            "rows": len(rows_for_coverage), "missing_rows": missing_sides,
            "complete": not missing_sides and bool(rows_for_coverage),
            "scope": "field presence only; actual independent side validation is separate"}
    ...
    if args.mode == "compensated" and not summary.get("eliminated_precision_coverage", {}).get("complete"):
        summary["P1_blockers"].append("eliminated_precision_not_persisted")
...
def main(argv=None):
    ...
    try:
        ...
        print("V8 PROTOTYPE NOT STARTED: " + type(exc).__name__ + ": " + str(exc), file=sys.stderr)
        return 2
