...
SCAPS_REF: dict[str, dict[float, dict[str, float]]] = {
    ...
    # SCAPS PDF p12 PVK/ETL interface defect density. Units are areal
    # (cm^-2); the legacy "_cm3" axis label was a misnomer. Phase E1.7
    # routes this sweep to ``DeviceStack.interface_defects[k]`` via the
    # new ``interface_defect_N_t_cm2`` sweep key — previously it routed
    # to the Phase 4a layer-trap-profile knob and the sweep showed flat
    # V_oc / 0 mV range / mismatch direction.
    "interface_defect_N_t_cm2": {
...
# SCAPS_REF axis keys to apply_sweep_point.updates keys.
_AXIS_TO_UPDATE_KEY = {
    ...
    # Phase E1.7 — drive the SCAPS interface defect density sweep through
    # the new ``interface_defect_N_t_cm2`` handler so it actually modulates
    # ``DeviceStack.interface_defects[k]`` SRV (was wired to the unrelated
    # Phase 4a layer-trap-profile via the legacy
    # ``interface_trap_density_cm3`` key — direction mismatched, range 0).
    "interface_defect_N_t_cm2": "interface_defect_N_t_cm2",
...
def write_report(
    ...
    lines.append(f"Compares SolarLab (`{config_path.as_posix()}` via "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/scaps_mirror_v2.yaml"))
    ...
    write_report(args.out_dir, base, sweep_summaries, args.config)
