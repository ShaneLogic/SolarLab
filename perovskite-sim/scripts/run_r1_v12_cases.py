...
def load_request(path, expected, output):
    ...
    if request.get("schema") != SCHEMA or request.get("scope") != SCOPE:
        raise ValueError("not a bounded V12 case request")
    if request.get("case_id") not in CASES:
        raise ValueError("case is outside the approved V12 set")
    ...
    if (request.get("kind") != kind or set(case) != set(axes) | {"times_s"}
        ...
        raise ValueError("V12 physical axes differ from the named case")
    if (type(case["intervals"]) is not int or type(case["nonlinear_factor"]) not in (int, float)
        ...
        raise ValueError("V12 physical axes cannot use boolean or string coercion")
    ...
    for key, value in (("inputs", INPUTS), ("certificate_limits", LIMITS),
        ...
        if request.get(key) != value:
            raise ValueError("V12 frozen requirement changed: " + key)
    ...
    if request.get("historical_matrix_sha256") != HISTORICAL_MATRIX_SHA256:
        raise ValueError("V12 original matrix provenance differs")
    ...
    if "required_pair_preparation_identity_sha256" not in request:
        raise ValueError("V12 explicit preparation identity requirement is absent")
...
def run(args):
    if not (sys.flags.isolated and sys.flags.no_site):
        raise ValueError("V12 requires an isolated Python -I -S process")
    if any(os.environ.get(key) != "1" for key in THREAD_KEYS):
        raise ValueError("V12 requires all five numerical thread limits to be 1")
    ...
    if (budget.get("schema") != "R1V12PrecisionBudgetV1"
        ...
        raise ValueError("V12 numerical/engineering budget binding differs")
    baseline = None
    ...
    summary = {"schema": "R1V12CaseRunV1", "scope": SCOPE, "case_id": request["case_id"],
    ...
    def guard():
        if source_snapshot(args.expected_commit) != initial_source:
            raise ValueError("source changed during V12 execution")
        for path, digest in ((args.request_file, args.request_sha256), (args.budget_file, args.budget_sha256)):
            if sha(path) != digest:
                raise ValueError("external V12 contract changed")
