...
def test_cigs_back_grading_raises_voc_without_jsc_collapse():
    ...
    common = dict(N_grid=40, n_points=21, V_max=0.8, illuminated=True)
    res_off = run_jv_sweep(base, **common)
    res_on = run_jv_sweep(graded, **common)

    J_off = np.asarray(res_off.J_forward)
    J_on = np.asarray(res_on.J_forward)
    ...
    m_off = compute_metrics(np.asarray(res_off.V_forward), J_off)
    m_on = compute_metrics(np.asarray(res_on.V_forward), J_on)
