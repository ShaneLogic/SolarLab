"""Single-ion DAE temporal refinement against the production MoL solver."""
...
def _biased_single_ion_problem():
...
def test_single_ion_be_contracts_to_production_mol_reference():
    grid, stack, model, initial = _biased_single_ion_problem()
    final_time_s = 1.0e-2
    mol = run_transient(
        grid,
    ...
    for errors in (carrier_errors, positive_ion_errors, potential_errors):
        values = np.asarray(errors)
        assert np.all(np.diff(values) < 0.0)
        np.testing.assert_allclose(values[1:] / values[:-1], 0.5, rtol=0.03)
    assert carrier_errors[-1] < 5.0e-9
