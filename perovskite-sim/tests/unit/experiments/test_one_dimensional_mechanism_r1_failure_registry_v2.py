...
LEGACY_ENTRIES = [entry for entry in ENTRIES if entry["observed_source_commit"] == "04170c4efcbcaa023a63fcee1f30bb41cceee0f8"]


def test_new_registry_retains_fourteen_observed_conditions_without_rewriting_history():
    assert len(LEGACY_ENTRIES) == 14
    assert sum(entry["control"] == "D" for entry in LEGACY_ENTRIES) == 6
    assert [sum(entry in LEGACY_ENTRIES for entry in V2[key]) for key in CATEGORIES] == [10, 4]
    ...
    assert sum(len(merged[key]) for key in CATEGORIES) == 40


def test_v6_strict_failure_retains_exact_case_and_incomplete_extent():
    added=[entry for entry in ENTRIES if entry not in LEGACY_ENTRIES]
    assert len(added)==1
    entry=added[0]
    assert entry["observed_source_commit"]=="a71f1fa6adef719202913ef3e2fc4c8c9297f7f2"
    assert (entry["control"],entry["intervals"],entry["nonlinear_factor"])==("B",256,.01)
    assert entry["refinement_substeps"]==[8,16,32]
    assert entry["fault"]=="none"
    assert entry["confirmation"]["rows_by_substeps"]=={"8":33,"16":65,"32":12}
    assert entry["confirmation"]["accepted_rows"]==110
    assert entry["confirmation"]["expected_complete_rows"]==227
    assert entry["confirmation"]["root_cause_established"] is False
...
def test_each_extra_case_matches_only_its_measured_axes_and_reports_source(entry):
    ...
    request["refinement_substeps"] = [16, 32, 64]
