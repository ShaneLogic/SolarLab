"""V8 prototype evidence must distinguish a real complete trace from a prefix."""
...
path = Path(__file__).resolve().parents[3] / "scripts/run_r1_v8_prototype.py"
...
def test_cost_can_compare_completed_failed_baseline_but_never_prefix():
    baseline = {"schema": "R1V8PrototypeRunV1", "mode": "baseline", "request_sha256": "a" * 64,
                "source_commit": "b" * 40, "source_content_sha256": "c" * 64,
    ...
    current["source_commit"] = "d" * 40
    assert "source_identity_differs" in runner.cost_comparison(current, baseline, "a" * 64)["reasons"]
