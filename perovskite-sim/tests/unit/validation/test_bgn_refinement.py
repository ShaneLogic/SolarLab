"""Registered incomplete-ionization plus BGN refinement contract."""
...
from perovskite_sim.validation.bgn_refinement import (
    _execution_protocol,
    run_incomplete_ionization_bgn_temperature_refinement,
)
...
LANE_ID = "incomplete-ionization-bgn-temperature-equilibrium-v1"
...
def _protocol(lane):
    ...
    return _execution_protocol(
        ...
        reference_density_m3=1.0e23,
        log_shape=0.5,
        conduction_band_fraction=0.5,
    )


def test_protocol_binds_bgn_law_temperature_levels_and_full_matrix():
    lane = _lane()
    ...
    assert protocol["band_gap_narrowing"] == {
        "conduction_band_fraction": 0.5,
...
def test_each_bgn_protocol_parameter_changes_content_hash():
    lane = _lane()
    baseline = _protocol(lane)

    for key, value in (
        ("reference_energy_eV", 0.01),
        ("reference_density_m3", 2.0e23),
        ("log_shape", 0.4),
        ("conduction_band_fraction", 0.6),
    ):
        changed = dict(baseline)
        changed["band_gap_narrowing"] = dict(baseline["band_gap_narrowing"])
        changed["band_gap_narrowing"][key] = value
        assert content_sha256(changed) != content_sha256(baseline)


def test_registered_executor_real_temperature_cell_matches_contract():
    lane = _lane()
    measurement = run_incomplete_ionization_bgn_temperature_refinement(
        lane,
        MatrixPoint(40, 1.0),
        ROOT,
    )
    ...
    donor_curve = observables["donor_ionized_fraction_temperature_curve"].values
    acceptor_curve = observables["acceptor_ionized_fraction_temperature_curve"].values
    narrowing_curve = observables["band_gap_narrowing_temperature_curve_eV"].values
    effective_gap_curve = observables["effective_band_gap_temperature_curve_eV"].values
    assert donor_curve == tuple(sorted(donor_curve))
    assert acceptor_curve == tuple(sorted(acceptor_curve))
    assert len(set(narrowing_curve)) == 1
    assert narrowing_curve[0] > 0.02
    assert effective_gap_curve == tuple(sorted(effective_gap_curve, reverse=True))
    assert quality["bgn_chemical_doping_law_verified"].values == (1.0,)
    assert quality["combined_topology_verified"].values == (1.0,)
    ...
    assert metadata["actual"]["expected_band_gap_narrowing_eV"] > 0.02
