...
from perovskite_sim.validation.dae_ion_interface_refinement import (
    _execution_protocol,
    run_single_ion_algebraic_interface_dae_transient,
)
...
def test_protocol_binds_combined_topology_and_synthetic_ion_parameters():
    lane = _lane()
    ...
    protocol = _execution_protocol(
        ...
        positive_ion_diffusion_m2_s=float(options["positive_ion_diffusion_m2_s"]),
        positive_ion_reference_m3=float(options["positive_ion_reference_m3"]),
        positive_ion_site_limit_m3=float(options["positive_ion_site_limit_m3"]),
        ...
        max_ion_logit_update=float(options["max_ion_logit_update"]),
        max_interface_logit_update=float(options["max_interface_logit_update"]),
    ...
    assert protocol["topology"]["mobile_ions"] == "single_positive_synthetic"
    ...
    assert protocol["topology"]["positive_ion_diffusion_m2_s"] == 1.0e-16
    assert protocol["topology"]["positive_ion_reference_m3"] == 1.0e22
    assert protocol["topology"]["positive_ion_site_limit_m3"] == 1.0e24
...
def test_registered_executor_rejects_ion_reference_at_site_limit():
    lane = _lane()
    incompatible = replace(
        ...
        options_json=json.dumps(
            {
                **lane.options,
                "positive_ion_reference_m3": lane.options["positive_ion_site_limit_m3"],
            }
        ),
    )
...
def test_registered_executor_real_cell_matches_exact_contract():
    lane = _lane()
    measurement = run_single_ion_algebraic_interface_dae_transient(
        lane,
        MatrixPoint(4, 1.0),
        ROOT,
    )
    ...
    assert quality["combined_ion_interface_topology_verified"].values == (1.0,)
    ...
    assert quality["minimum_interface_state_relative_motion"].values[0] > 1.0e-3
    ...
    assert metadata["actual"]["minimum_positive_ion_occupation_margin"] > 0.0
