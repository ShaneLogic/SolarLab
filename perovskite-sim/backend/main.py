    ...
    if kind == "jv":
        ...
        def _run(reporter: ProgressReporter) -> dict:
            result = run_tpv(
                ...
                rtol=float(p.get("rtol", 1e-4)),
                atol=float(p.get("atol", 1e-6)),
                voltage_atol=float(p.get("voltage_atol", 1e-9)),
                max_step=float(p["max_step"]) if p.get("max_step") is not None else None,
                max_voltage_error_V=float(p.get("max_voltage_error_V", 1e-6)),
                max_current_error_A_m2=float(p.get("max_current_error_A_m2", 0.05)),
                experiment_protocol=experiment_protocol,
                protocol_mode=protocol_mode,
                progress=lambda stage, cur, tot, msg: reporter.report(stage, cur, tot, msg),
            )
