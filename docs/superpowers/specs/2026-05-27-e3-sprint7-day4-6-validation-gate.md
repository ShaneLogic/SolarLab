...
1. **Newton stability.** Interface defect N_t sweep no longer hangs. χ-step-anchored dark-eq init keeps cross-flux ≈ 0 at initial state.
...
2. **Interface defect N_t collapsed FURTHER** (210 → 0 mV). Defect density sweep has ZERO effect on V_oc because χ-step-constrained iface_state plus χ-step-consistent ni_eff² makes the SRH numerator (np − ni²) invariant to N_t.

## Diagnosis

The χ-step cross-flux ties n_1s, n_2s into a rigid ratio. As ΔE_c sweeps:
- iface_state densities scale proportionally
- SRH pair products n_1s · p_2s = n_1s · p_1s · exp(−ΔE_v/V_T) = ni²_R · exp(−ΔE_v/V_T)
- ni_eff² also = ni²_R · exp(−ΔE_v/V_T) by construction
...
Same mechanism kills interface defect N_t sweep: defect N_t enters via v_n, v_p (calibration_factor). When the np product nearly equals ni_eff² (always, due to χ-step constraint), the SRH rate ≈ 0 regardless of v_n, v_p magnitude.

## Five-prototype failure pattern (cumulative)

| Prototype | CBO | ETL doping | Interface N_t | Newton |
|---|---|---|---|---|
...
- Closing ETL doping → loses CBO + interface N_t
...
| **(C) Park** — accept three structural gaps as documented (ETL, interface N_t, CBO+CBO tradeoff). Ship main as Phase H. | 0 days | none; partner already has Phase H |
