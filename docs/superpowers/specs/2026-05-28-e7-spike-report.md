...
Per-point V<sub>oc</sub> / J<sub>sc</sub>:

| N<sub>D</sub> (cm⁻³) | V<sub>oc</sub> (V) | J<sub>sc</sub> (A/m²) | Bracketed | Note |
|---:|---:|---:|---|---|
...
| 5e18 | 1.1105 | **131** | ✓ | J<sub>sc</sub> collapse |
| 1e19 | 1.1478 | **5.3** | ✓ | J<sub>sc</sub> collapse (V<sub>oc</sub> unreliable) |

**Trend across physical regime [1e16, 5e17]**: V<sub>oc</sub> rises monotonically
+27 mV across 1.5 decades — direction matches SCAPS (V<sub>oc</sub> RISES with N<sub>D</sub>).
SCAPS reference range over PDF page 2 was 34 mV; SolarLab gives 27 mV in
the physical sub-range. Closure ~80%.

**Anomalies at N<sub>D</sub> ≥ 1e18**: J<sub>sc</sub> collapses by 100×–10000×. Likely heavy
donor doping kills collection (PVK becomes too n-type, hole extraction
breaks). NOT a V<sub>oc</sub> direction artifact — separate physics question, out
of E7 scope unless partner xlsx includes these high-N<sub>D</sub> points.
...
- v<sub>th</sub> = 1e5 m/s, N<sub>t</sub> = 1e18 m⁻³
- τ_CB = τ_VB = 1e-4 s
...
equilibrium → V<sub>oc</sub>-level injection → asymmetric n-rich / p-rich:
...
| V<sub>oc</sub>-level | (1e23, 1e23) | 4.89e26 | 4.89e26 | 1.0000 |
| asymmetric n-rich | (1e23, 1e18) | 6.47e21 | 6.47e21 | 1.0000 |
| asymmetric p-rich | (1e18, 1e23) | 6.47e21 | 6.47e21 | 1.0000 |

**Max deviation: 0.00%.**

The CB+VB defect pair has identical (σ_n, σ_p, N<sub>t</sub>) with mirror-image
...
Per-config Nd_ETL closure (V<sub>oc</sub> range over the working subset of bracketed
+ physical points):

| Config | S values (m/s) | Bracketed | V<sub>oc</sub> range (SL) | V<sub>oc</sub> range (SCAPS) | Closure |
|---|---|---:|---:|---:|---:|
...
Per-point V<sub>oc</sub> (working subset only):

| N_d (cm⁻³) | V<sub>oc</sub> Dirichlet | V<sub>oc</sub> Robin mod | V<sub>oc</sub> Robin strong | V<sub>oc</sub> SCAPS |
|---:|---:|---:|---:|---:|
...
1. **No 2.1 V unphysical branch at V<sub>max</sub>=1.6 V in any config.** The 2.1 V
   E6.5 branch was a V<sub>max</sub>=2.5 V artifact, not a default-sweep concern.
...
2. **High-Nd regime (1e15-1e20) is identical across all three configs**
   (V<sub>oc</sub> differs by < 2 mV). Robin S choice does not move the working
   regime. The Nd_ETL under-sensitivity is **bulk-limited**, not
   contact-limited.

3. **Strong Robin (S=10/0.1) inflates low-Nd V<sub>oc</sub>** to unphysical-ish
...
4. **Root cause** (working hypothesis): the E1.5 cross-carrier interface
   ...
   clamps V<sub>oc</sub> to ~1.07 V across most of the sweep range. No contact
...
1. **Park Y2** — accept Nd_ETL 30% closure as ship-state. Document as
   ...
   non-blocking".
2. **Lower PVK/ETL SRV calibration** — would raise V<sub>oc</sub> ceiling but
...
3. **Two-preset split with Y1-style approach** — `scaps_mirror_v2_loSRV.yaml`
   for Nd_ETL sweep with reduced PVK/ETL N<sub>t</sub>. Risk: still doesn't make
...
**Recommended path**: ship Y1 (bulk N<sub>t</sub> closure) + park Y2 with
...
3. **If user approves Y1-only ship**: invoke writing-plans skill for
   Y1 (bulk N<sub>t</sub> SRV tune) — small enough that an implementation plan
...
| Variant | V<sub>oc</sub> baseline | V<sub>oc</sub> range across sweep | Closure |
|---|---:|---:|---:|
| baseline (N<sub>t</sub>=1e12) | 1.0718 V | 0.07 mV | 0.18 % |
| lo (N<sub>t</sub>=1e10) | 1.0873 V | 0.10 mV | 0.27 % |
| lo (N<sub>t</sub>=1e8) | 1.0874 V | 0.10 mV | 0.27 % |

Lowering PVK/ETL N<sub>t</sub> by 10000× lifts V<sub>oc</sub> baseline 15 mV but does
...
| Variant | V<sub>oc</sub> baseline | V<sub>oc</sub> range across sweep |
|---|---:|---:|
...
combined variant: Auger=0, B<sub>rad</sub>=0, PVK/ETL N<sub>t</sub>=1e8 (10000× lower).

| N<sub>t</sub> (cm⁻³) | V<sub>oc</sub> SL | V<sub>oc</sub> SCAPS |
|---:|---:|---:|
| 1e9 | 1.524 V | 1.168 V |
...
V<sub>oc</sub> range SolarLab: 231 mV. SCAPS: 39 mV. Direction matches
(V<sub>oc</sub> falls as N<sub>t</sub> rises). Closure 599 % (over-shoots because we
...
*more* sensitive to N<sub>t</sub> than SCAPS's, but masked in production by
...
that might explain the V<sub>oc</sub> ceiling cascade. Identified one apparent
...
10×, 100×, 1000× lower). Base J-V V<sub>oc</sub> measured per variant.

| Variant | A* (A/m²·K²) | V<sub>oc</sub> |
|---|---:|---:|
...
**ΔV<sub>oc</sub> across all variants: 0.0 mV.**
...
is lowered 1000×. The "TE formula difference" between SCAPS (v<sub>th</sub>) and
SolarLab (RD) is invisible to V<sub>oc</sub> here because SolarLab's effective TE
...
the bulk N<sub>t</sub> mask via the recombination cascade, and part of the
−87 mV base V<sub>oc</sub> absolute gap.
