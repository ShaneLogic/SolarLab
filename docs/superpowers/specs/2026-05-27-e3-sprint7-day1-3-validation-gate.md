...
**Status:** MIXED — ETL doping closure +88 % but CBO collapsed, Newton hang on interface defect N_t sweep
**Branch:** `e3-interface-plane-state`
**Input:** `outputs/scaps_validation_e3_iface_plane/` (partial — killed at interface defect N_t sweep after 36 min hang)

## Partial result vs all five prior prototypes

| Sweep | Legacy | BBD | Thin-shell | Single PV | Full PV | **Iface-state** | SCAPS |
|---|---|---|---|---|---|---|---|
| Base V_oc | 1.069 | 1.054 | 1.088 | 1.0905 | 1.0905 | **1.0905** | 1.168 |
| CBO range (mV) | 782 ✓ | 847 ✓ | 16 ✗ | 663 ~ | 663 ~ | **18 ✗** | 918 |
| ETL doping range | 1075 ✗ | 1542 ✗ | 17 ✓ | 1419 ✗ | 1 ✗ | **15 ✓** | 137 |
| PVK doping | 34 | 21 | 48 | 50 | stuck | **49** | 34 |
| Interface N_t | 210 | 208 | 206 | 8 ✗ | n/a | **hang ✗** | 282 |
| Bulk N_t | 0 | 0 | 0 | 0 | 0 | **0** | 39 |
...
| Interface defect N_t ≥ 150 mV | ≥ 150 | hang (untested) | **FAIL** (Newton stability) |
...
### Interface defect N_t sweep Newton hang

At certain N_t values (likely 1e15-1e17 cm⁻²), the SRH consumption rate exceeds the TE refill rate by orders of magnitude. The iface_state ODE becomes super-stiff: τ_SRH << τ_TE.
...
3. Tighter v_th_eff calibration — current 1e-2 m/s may need to drop further at high N_t.

## Design choices that landed

- `_DEFAULT_V_TH_MS = 1.0e-2` — surface-recombination-velocity scale rather than thermal 1e5. Textbook v_th makes Newton diverge at diode knee (V_app ≈ 0.5 V). Sprint 7 Day 4+ revisits with QSS reduction.
- Bulk drain coupling — TE flux subtracted from bulk dn/dp at eval_n/eval_p nodes. Without this, V_oc was bit-identical to legacy (interface-state was decoupled from bulk dynamics).
...
| 5-6 | Add cross-interface χ-step coupling: TE flux for n_1s↔n_2s with exp(−ΔE_c/V_T) factor (paper eq 14a) |
