...
|---|---|---|---|---|---|---|
| Base V_oc | 1.069 | 1.054 | 1.088 | **1.0905 V** | 1.168 | within envelope ✓ |
...
| **Interface defect N_t** | **210** | 208 | 206 | **8 mV** | 282 | **COLLAPSED 74 % → 3 %** ✗ |
| Bulk N_t | 0 | 0 | 0 | 0 | 39 | unchanged (still blocked) |
| Bulk E_t | 2 | 1 | 3 | 3 | 0 | flat both |
...
Factor ramps from ~0 at low bias to 1 near V_bi. Suppresses p_iface
...
weakens → only ETL doping survives because it shifts V_bi directly.

The heavy-doping assumption is mathematically equivalent to assuming
ALL band-bending happens in PVK with full V_bi − V_app drop. For
...
| Prototype | CBO | ETL doping | Interface N_t |
|---|---|---|---|
...
partner report alongside the Phase G base V_oc gap (74 mV) and Phase
...
Ship the main-branch state (CBO 85 %, PVK/ETL N_t 74 %, base V_oc
within envelope) as the final partner deliverable. Document the ETL
doping over-sensitivity, PVK doping direction, and bulk N_t mismatch
