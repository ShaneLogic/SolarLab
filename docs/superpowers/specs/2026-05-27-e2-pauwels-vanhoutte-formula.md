...
V_2 then derived from V_bi = V_1 + V_2 − V.

**Key insight:** V_1 depends ONLY on band offsets (ΔE_c, ΔE_g) — NOT
...
v_th of the two layers (per SCAPS Manual §3.8).
...
- V_1 = V_bi − V_app (light-doping PVK side absorbs all band-bending)
...
- ΔE_v = ΔE_g − |ΔE_c| ... need to compute for our stack

For ETL Eg=4 eV, PVK Eg=1.53 eV: ΔE_g = 4 − 1.53 = 2.47 eV.
ΔE_c = chi_ETL − chi_PVK = 4 − 3.84 = 0.16 eV (positive: ETL chi
deeper, electron well attracts).
ΔE_v = ΔE_g + ΔE_c = 2.47 + 0.16 = 2.63 eV ≫ 0 → no inversion.
...
- "Sem. 2" in our case = PVK (light p-doped) → V_2 = V_bi − V_app

So PVK absorbs all band-bending V_bi − V_app, consistent with intuition.

p_iface (PVK side) = p_2 · exp(V_2 + V) = p_2 · exp(V_bi)  (since V_2 = V_bi − V).

Wait this is constant (independent of V_app) under the substitution
V_2 + V = V_bi. Let me re-derive in our solver's convention:
...
φ_iface − φ_bulk_PVK = V_bi − V_app (full band-bending).
...
E_v_iface_PVK = E_v_bulk_PVK − q·(φ_iface − φ_bulk_PVK)
            = E_v_bulk_PVK − q·(V_bi − V_app)

So p[iface PVK side] = p[bulk PVK] · exp(−q·(V_bi − V_app)/kT)
                    = p[bulk PVK] · exp(−(V_bi − V_app)/V_T)

This is what we want. The depletion factor for the PVK-side hole at
interface is exp(−(V_bi − V_app)/V_T). DOES NOT depend on N_D_ETL
(except through V_bi which depends on Fermi-level alignment, weak
log-dependence).

Compare BBD: used exp((φ[idx] − φ[idx_L])/V_T) where φ[idx] − φ[idx_L]
is the LOCAL grid potential drop ≈ V_bi − V_app for grids fine enough
to resolve depletion zone. So BBD ALMOST gets it right but:
- Grid-discretisation noise contaminates the φ difference.
- For coarse grids, the local drop ≠ total V_bi.
- The "near-equivalence" is what made probe show 2.8× reduction at
  high-N_D but the formula diverges from PV at low N_D where grid
  cannot resolve the wider depletion.

**Pauwels-Vanhoutte implementation strategy:**

Use V_bi − V_app (a GLOBAL quantity from cached mat.V_bi_eff and the
RHS call's V_app kwarg) instead of local grid potential differences:
...
This formula should:
1. **Preserve CBO sensitivity:** V_bi depends on band offsets (ΔE_c
   shifts Fermi-level alignment → V_bi changes). So CBO sweep is
   reflected through V_bi → V_1 → p_iface → R.
2. **Reduce ETL doping sensitivity:** N_D_ETL changes V_bi only via
   ...
   into ETL at low N_D.
3. **Preserve PVK doping direction:** N_A_PVK changes V_bi
...
| 4 | Local regression + ad-hoc V_oc probe + N_D_ETL spot check |
