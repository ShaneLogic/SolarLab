...
|---|---|---|
| **Device-level** (V_bi, Phi, mode) | partial | V_bi=1.30 V is SolarLab-derived; SCAPS does not declare V_bi explicitly |
| **HTL layer** (11 fields) | ✓ ALL MATCH | thickness, Eg, chi, eps, mu, Nc/Nv, Nd/Na, v_th identical |
| **PVK layer** (14 fields) | ✓ ALL MATCH | inc. B_rad=1e-12, C_n/p=2.3e-29 |
...
| # | PDF defect | σ_n=σ_p (cm²) | Distribution | Et (eV) | N_t (cm⁻³) | YAML status |
|---|---|---|---|---|---|---|
...
> "若 SCAPS 输入 peak defect density 而不是 total defect density, 保持 characteristic energy 为 0.1 eV, 按比例缩放 N_peak = 5.64×10⁸ × N_t / (1×10¹²)"
...
| Sweep | V_oc range | Notes |
|---|---|---|
| CBO (ΔE_C from -1.0 to +0.7) | **918 mV** | 0.332 → 1.250, base at ΔE_C=-0.16 → 1.1676 V |
...
| HTL/PVK defect N_t (1e9 to 1e15) | **0.5 mV** | essentially flat — interface 1 is inactive |
| PVK-CB defect N_t (1e9 to 1e15) | 39 mV | base 1e12 → 1.1676; 1e15 → 1.129 |
| PVK-VB defect N_t (1e9 to 1e15) | 11 mV | base 1e12 → 1.1676; 1e15 → 1.157 |
| PVK/ETL defect N_t (1e9 to 1e15) | **282 mV** | dominant interface; base 1e12 → 1.1676; 1e15 → 0.968 |
| HTL/PVK defect E_t (0.01 to 0.6) | <0.1 mV | flat |
| PVK-CB defect E_t (0.01 to 0.6) | 0.4 mV | nearly flat |

Base SCAPS perf: V_oc=1.1676 V, J_sc=26.28 mA/cm², FF=86.99%, η=26.69%.

## Implications

1. **Base layer params match perfectly.** The 99 mV V_oc gap (SolarLab 1.069 vs SCAPS 1.1676) does NOT come from layer band/mobility/doping mistuning.
2. **Defect inventory is the smoking gun.** YAML declares 2 of 4 defects. The dominant interface defect (PVK/ETL Gaussian → SCAPS V_oc range 282 mV) is encoded with wrong σ (4 orders), wrong units (cm⁻² vs cm⁻³), wrong distribution (Single vs Gaussian), wrong Et below CB position, and a 1e-4 calibration factor hiding all of the above.
3. **ETL doping gap (1075 vs 137 mV) is partly explained by defect-block mismatch.** The cross-carrier P-V SRH in `_apply_interface_recombination` reads bulk-interior carrier densities; this couples linearly with N_D_ETL when the defect's σ·N_t product is set 4 orders too large via the calibration trick. Correcting σ + N_t units + distribution may narrow the gap WITHOUT touching the solver.
4. **PVK-VB defect is missing.** Adds a recombination channel SCAPS uses that SolarLab does not, contributing to V_oc shortfall.
5. **HTL/PVK defect is missing but inactive in SCAPS** (V_oc sweep flat). Adding it costs nothing structurally; matches partner model completeness.

## Recommended action sequence

### Step 1 (1 hr): Stage 1 fix — match defect inventory
- Add `bulk_defect` for PVK-VB (σ=1e-15, Single, Et=0.1 above VB, N_t=1e12) to PVK layer
- Add HTL/PVK interface defect (σ=1e-19, Single, Et=0.6, N_t=1e12 cm⁻³ — note volumetric)
...
- Compare new SolarLab base V_oc vs SCAPS 1.1676 V
- Compare ETL doping range — does correcting σ + units narrow 1075 → 137 mV?
- Compare PVK/ETL N_t sweep — does it hit 282 mV without calibration_factor?
