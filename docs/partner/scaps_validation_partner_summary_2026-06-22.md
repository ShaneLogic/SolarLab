...
![ETL/PVK conduction-band offset (delta E~C~). Filled purple diamonds are the genuine SS interface-states solve over the flat-band region (delta E~C~ >= -0.16 eV); hollow diamonds are the certified transient fallback over the deep-CBO arm (delta E~C~ <= -0.2 eV), where the collapsed junction is unreachable by the SS Newton. The SS curve sits on the transient throughout — CBO is a band-offset effect, so SS approx the transient (not interface-limited).](../figures/scaps_ss_compare/sweep_CHI_ETL.png){width=86%}
...
**The steady-state interface-states series.** Two things distinguish it from the transient de-spike curves. (i) **Direction:** it recovers the ETL-donor-doping rising arm that both transient fractions get backwards (Section 4.1) — the only sweep no de-spike value can fix. (ii) **Calibration:** the interface-plane channel was over-strong (base -61 mV, Nd_ETL ~2x over, HTL/PVK N~t~ ~12x over); a per-interface calibration (`iface_state_calibration_factor` 0.02 HTL/PVK, 0.10 PVK/ETL; Section 5.3) brings the base to -0.1 mV, Nd_ETL to 0.84x, and HTL/PVK N~t~ to 1.0x of SCAPS while holding PVK/ETL N~t~ at 63 %, all directions matched. The transient f=0.53/0.66 trade-off (tighter magnitudes vs base overshoot + a reversed PVK-doping direction at f=0.66) is unchanged from the de-spike analysis.
