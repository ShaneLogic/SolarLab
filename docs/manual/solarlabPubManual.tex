...
genuinely binds, and where it binds hard the settle becomes sharply more
expensive. An illuminated settle at \(V=0.9\,\mathrm{V}\) costs
\(0.17/0.47/1.21\,\mathrm{s}\) at \(N_\mathrm{grid}=20/45/90\) with the
superseded bound and \(0.25/0.52/{>}240\,\mathrm{s}\) with the
normalized one --- a cliff at fine mesh, which puts mesh-convergence
studies out of reach with the flag on (Chapter 17). Configurations
...
\item
  ...
  identified the Richardson constant and the effective density of states
  not being drawn from a common effective mass, so that
  \(v_R = A^{*}T^{2}/(qN_C)\) was not the \(\sqrt{k_BT/2\pi m^{*}}\) it
  reduces to when both derive from one \(m^{*}\) but 4.6x too small at
  the hole-transport layer --- enough to throttle a near-insulating
  contact into a non-finite solve. That inconsistency has since been
  fixed by deriving \(A^{*}\) from each layer's own DOS, and it is no
  longer the obstacle.

  The second pass found the obstacle that remains, and it is numerical
  rather than physical. Once the bound is normalized it genuinely binds,
  and where it binds hard the settle becomes very much more expensive
  --- sharply, not gradually. An illuminated settle at
  \(V=0.9\,\mathrm{V}\) on the mirrored configuration takes \(0.17\),
  \(0.47\) and \(1.21\,\mathrm{s}\) at \(N_\mathrm{grid} = 20\), \(45\)
  and \(90\) with the superseded bound; with the normalized one it takes
  \(0.25\), \(0.52\) and \emph{more than 240} --- a cliff at fine mesh
  rather than a constant factor. Mesh-convergence work, which is exactly
  what a validation study needs, is therefore not currently practical
  with the flag on. Making it the default awaits solver work on the
  conditioning of the capped face (the smooth magnitude blend the
