...
Two slow-suite tests fail at the certified commit; both failures
pre-date the 2026-07 revision (verified by running the identical tests
at the 2026-07-22 baseline commit, which fails identically) and are
tracked as open issues rather than hidden:

\begin{itemize}
\tightlist
\item
  \texttt{test\_grading\_cigs\_notch.py::test\_cigs\_back\_grading\_raises\_voc\_without\_}
  \texttt{jsc\_collapse} --- the graded-CIGS regression sweep hits a
  coupled-solver non-convergence at \(V_\mathrm{app} = 0.4\) V (solver
  robustness issue on that configuration, not a physics-assertion
  failure);
\item
  \texttt{test\_autoloop\_boulder.py::test\_boulder\_sweep\_real} --- an
  autoloop integration test whose expected gap proposal
  (\texttt{trend:Nd\_ETL:V\_oc}) no longer exists after the
  interface-channel calibration improved that trend; the test
