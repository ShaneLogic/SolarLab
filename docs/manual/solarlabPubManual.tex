...
whole-flux form is an empirical crowding regularization with the correct
saturation limit (Chapter 17).
...
Selberherr 1984). The scheme rests on three implementation details:
...
\texttt{voc\_bracketed} flag rather than metrics extrapolated from a
sweep that does not bracket the crossing. The hysteresis index compares
scan directions as
...
Four model-formulation limitations apply, each flagged at its point of
