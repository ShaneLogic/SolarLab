...
Input specification is a dominant source of simulation error. SolarLab cannot
infer whether a parameter comes from measurement, literature, fitting, or
exploratory screening, so each field should be recorded with its source and
value.
...
and the contact equilibrium densities) are left unchanged. The
...
and where it binds hard the settle becomes sharply more expensive. An
illuminated settle at $V=0.9\,\mathrm{V}$ costs $0.17/0.47/1.21\,\mathrm{s}$
at $N_\mathrm{grid}=20/45/90$ with the superseded bound and
$0.25/0.52/{>}240\,\mathrm{s}$ with the normalized one — a cliff at fine
mesh, which puts mesh-convergence studies out of reach with the flag on
(Chapter 17). Configurations without per-layer effective-DOS data are
...
field-dependent $E_{00}(F)$ evaluated per step, which is outside the present
scope.
...
2. **Self-consistent reabsorption** (full tier): $B_\mathrm{rad}$ keeps its
   ...
   B_\mathrm{rad}\,(np-n_i^2)\, dx\big]/d$. The net-emission integrand makes the
...
Pazos-Outón et al.). The recycling gain depends on the internal radiative
efficiency, parasitic absorption, and out-coupling, so the window is a
regression gate for the canonical preset rather than a general MAPbI3
figure.
...
- **Non-negativity clamp (model-specific safeguard, defect-scoped).** At
  ...
  suppress the resulting spurious generation (of order 10 A/m$^2$ at
...
- **Peak-density residual scaling**: residuals are normalized by the global
  ...
  near $10^{-10}$ while a physical imbalance registers at order unity, and
...
- **Certified fallbacks**: where the TE-cap kink makes Newton stall at an
  ...
  residual scale; an uncertifiable point raises an error rather than
...
are handled by the transient engine rather than the Newton driver:
near-insulating contact doping produces no $J = 0$
...
measurement convention. The baseline state is the settled dark
...
must be replaced before using the tandem driver for material-specific
predictions.
...
- the *default* thermionic-emission bound is density-weighted and therefore
  ...
  Two characterisation passes were run before promoting it, and it stays
  opt-in because each found a cost the previous had missed. The first
  identified the Richardson constant and the effective density of states not
  being drawn from a common effective mass, so that $v_R = A^{*}T^{2}/(qN_C)$
  was not the $\sqrt{k_BT/2\pi m^{*}}$ it reduces to when both derive from one
  $m^{*}$ but 4.6x too small at the hole-transport layer — enough to throttle
  a near-insulating contact into a non-finite solve. That inconsistency has
  since been fixed by deriving $A^{*}$ from each layer's own DOS, and it is
  no longer the obstacle.

  The second pass found the obstacle that remains, and it is numerical
  rather than physical. Once the bound is normalized it genuinely binds, and
  where it binds hard the settle becomes very much more expensive — sharply,
  not gradually. An illuminated settle at $V=0.9\,\mathrm{V}$ on the mirrored
  configuration takes $0.17$, $0.47$ and $1.21\,\mathrm{s}$ at
  $N_\mathrm{grid} = 20$, $45$ and $90$ with the superseded bound; with the
  normalized one it takes $0.25$, $0.52$ and *more than 240* — a cliff at
  fine mesh rather than a constant factor. Mesh-convergence work, which is
  exactly what a validation study needs, is therefore not currently
  practical with the flag on. Making it the default awaits solver work on
  the conditioning of the capped face (the smooth magnitude blend the
  steady-state driver already uses is the obvious starting point), not a
  further re-measurement;
...
8. L. A. A. Pettersson, L. S. Roman, and O. Inganäs, "Modeling photocurrent
